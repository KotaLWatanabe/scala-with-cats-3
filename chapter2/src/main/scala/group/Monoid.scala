...
object MonoidInstances:
  ...
  given intAdditionMonoid: Monoid[Int] with
    ...
    override def combine: (Int, Int) => Int =
      SemigroupInstances.intAdditionSemigroup.combine

  given stringMonoid: Monoid[String] with
    override def empty: String = ""
    override def combine: (String, String) => String =
      SemigroupInstances.stringSemigroup.combine
  ...
  given booleanXorMonoid: Monoid[Boolean] with
    override def empty: Boolean = false
    override def combine: (Boolean, Boolean) => Boolean = (b1, b2) =>
      (b1 && !b2) || (!b1 && b2)
  ...
  given setSymDiffMonoid[A]: Monoid[Set[A]] with
    override def empty: Set[A] = Set.empty
    override def combine: (Set[A], Set[A]) => Set[A] = (s1, s2) =>
